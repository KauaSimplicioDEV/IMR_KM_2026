...
# Velocidade máxima de cada roda (pixels/s)
V_RODA_MAX = 150.0
...
class DiffDriveRobot:
    def __init__(self, x, y, theta=0.0, wheelbase=30.0, radius=15.0):
        ...
        # Velocidades individuais das rodas (para telemetria)
        self.v_left = 0.0
        self.v_right = 0.0
...
def main():
    ...
    pygame.display.set_caption("Aula 01 - Exercicio 1: Controle Individual das Rodas")
    ...
    while running:
        ...
        # Leitura do teclado para controle individual das rodas
        # Roda esquerda: W (frente) / S (trás)
        # Roda direita:  I (frente) / K (trás)
        ...
        # Aplica comando via cinemática diferencial (roda esquerda/direita)
        ...
        info_txt = [
            ...
            f"Rodas: v_esq = {robot.v_left:.1f} px/s | v_dir = {robot.v_right:.1f} px/s",
            f"Resultante: v = {robot.v:.1f} px/s | omega = {robot.omega:.2f} rad/s",
            "Controles: W/S = roda esquerda | I/K = roda direita",
            "Dica: W+K (ou S+I) = giro no proprio eixo | so W ou so I = giro em torno da roda travada",
        ]
